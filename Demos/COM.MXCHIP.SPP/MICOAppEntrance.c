...
void appRestoreDefault_callback(mico_Context_t *inContext)
{
  ...
  inContext->flashContentInRam.appConfig.USART_BaudRate = W2S_BAUD_RATE;
...
OSStatus MICOStartApplication( mico_Context_t * const inContext )
{
  ...
  // JB
  // Fixed the baudrate : comm with Main MCU
  uart_config.baud_rate    = W2S_BAUD_RATE;
